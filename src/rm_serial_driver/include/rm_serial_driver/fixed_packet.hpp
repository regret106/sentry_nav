...
class FixedPacket {
 ...
 public:
  ...
  bool loadData(T const &data, int index) {
    // 越界检测
    // 检查索引是否大于0，并且索引加上数据长度是否小于容量
    if (index > 0 && ((index + data_len) < (capacity))) {
      // 如果没有越界，将数据复制到缓冲区的指定位置
      memcpy(buffer_ + index, &data, data_len);
      // 返回true表示数据加载成功
      return true;
    }
    // 如果索引越界，抛出越界异常
    throw std::out_of_range("loadData: Index out of range");
    // 由于异常抛出后不会执行到这一行，这一行实际上不会被执行到
    return false;
  }
  ...
  bool unloadData(T &data, int index) {
    ...
    throw std::out_of_range("unloadData: Index out of range");
    return false;
  }
